...
TGraphAsymmErrors RestoreBinning(TGraphAsymmErrors const& src, TH1F const& ref) {

  TH1F res = ref;
  res.Reset();
  TGraphAsymmErrors g( &ref );

  for (int x = 0; x < g.GetN(); ++x) {
    g.GetY()[x]=src.GetY()[x];
    g.SetPointError(x, ref.GetBinWidth(x+1)/2, ref.GetBinWidth(x+1)/2, src.GetErrorYlow(x), src.GetErrorYhigh(x) );
  }
  return g;
}
...
int main(int argc, char* argv[]) {
  ...
  config.add_options()
    ...
    ("asym",
      po::value<bool>(&asym)->default_value(asym)->implicit_value(true),
      "Produce TGraphAsymmErrors instead of TH1F")
    ("samples",
  ...
  map<string, map<string, TGraphAsymmErrors>> g_pre_shapes;
  ...
  for (auto bin : bins) {
    ...
    TGraphAsymmErrors tmp( &pre_shapes[bin]["data_obs"] );
    g_pre_shapes[bin]["data_obs"] = tmp;

    for (auto proc : cmb_bin.process_set()) {
      pre_shapes[bin][proc] =
          cmb_bin.cp().process({proc}).GetShapeWithUncertainty();
      g_pre_shapes[bin][proc] = cmb_bin.cp().process({proc}).GetShapeWithUncertaintyAsym();
    }

    // The fill total signal and total bkg hists
    pre_shapes[bin]["TotalBkg"] =     cmb_bin.cp().backgrounds().GetShapeWithUncertainty();
    g_pre_shapes[bin]["TotalBkg"] =   cmb_bin.cp().backgrounds().GetShapeWithUncertaintyAsym();
    pre_shapes[bin]["TotalSig"] =     cmb_bin.cp().signals().GetShapeWithUncertainty();
    g_pre_shapes[bin]["TotalSig"] =   cmb_bin.cp().signals().GetShapeWithUncertaintyAsym();
    pre_shapes[bin]["TotalProcs"] =   cmb_bin.cp().GetShapeWithUncertainty();
    g_pre_shapes[bin]["TotalProcs"] = cmb_bin.cp().GetShapeWithUncertaintyAsym();
    ...
    if (datacard != "") {
      ...
      for (auto & it : g_pre_shapes[bin]) {
        it.second = RestoreBinning(it.second, ref);
      }
    }
    ...
    outfile.cd();
    if (asym){
      for (auto& iter : g_pre_shapes[bin]) {
  ...
  if (postfit) {
    ...
    map<string, map<string, TGraphAsymmErrors>> g_post_shapes;
    ...
    for (auto bin : bins) {
      ...
      TGraphAsymmErrors tmp( &post_shapes[bin]["data_obs"] );
      g_post_shapes[bin]["data_obs"] = tmp;
      ...
      for (auto proc : cmb_bin.process_set()) {
        ...
        g_post_shapes[bin][proc] = cmb_proc.GetShapeWithUncertaintyAsym();
      ...
      if (asym){
	for (auto & iter : g_post_shapes[bin]) {
